...
describe('OpenYearlyNoteCommandHandler', () => {
    let commandHandler: OpenYearlyNoteCommandHandler;
    ...
    beforeEach(() => {
        ...
        commandHandler = new OpenYearlyNoteCommandHandler(dateManagerFactory, settingsRepositoryFactory, viewModel, calendarViewModel);
    });
    ...
    describe('execute', () => {
        ...
        it('should call the openNote method on the view model with the period from the date manager and select the note', async () => {
            ...
            // Assert
            expect(viewModel.openNote).toHaveBeenCalledWith(ModifierKey.None, year);
            expect(calendarViewModel.setSelectedPeriod).toHaveBeenCalledWith(year);
        });
    });
});