...
import { OpenQuarterlyNoteCommandHandler } from './open-quarterly-note.command-handler';

describe('OpenQuarterlyNoteCommandHandler', () => {
    let commandHandler: OpenQuarterlyNoteCommandHandler;
    ...
    beforeEach(() => {
        ...
        commandHandler = new OpenQuarterlyNoteCommandHandler(dateManagerFactory, settingsRepositoryFactory, viewModel, calendarViewModel);
    });
    ...
    describe('execute', () => {
        ...
        it('should call the openNote method on the view model with the period from the date manager and select the note', async () => {
            ...
            // Assert
            expect(viewModel.openNote).toHaveBeenCalledWith(ModifierKey.None, quarter);
            expect(calendarViewModel.setSelectedPeriod).toHaveBeenCalledWith(quarter);
        });
    });
});